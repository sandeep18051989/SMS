    ...
    public partial class Task
    {
        ...
        private ITask CreateTask(ILifetimeScope scope)
        {
            ...
            if (this.Enabled)
            {
                ...
                {
                    object instance;
                    if (!ContextHelper.Current.ContainerManager.TryResolve(type2, scope, out instance))
                    {
                        //not resolved
                        instance = ContextHelper.Current.ContainerManager.ResolveUnregistered(type2, scope);
                    }
        ...
        public void Execute(bool throwException = false, bool dispose = true, bool ensureRunOnOneWebFarmInstance = true)
        {
            ...
            try
            {
                ...
                if (ensureRunOnOneWebFarmInstance)
                {
                    ...
                    {
                        var machineNameProvider = ContextHelper.Current.ContainerManager.Resolve<IMachineNameProvider>("", scope);
            ...
            catch (Exception exc)
            {
                ...
                //log error
                var systemLogger = ContextHelper.Current.ContainerManager.Resolve<ISystemLogService>("", scope);
                systemLogger.Error(string.Format("Error while running the '{0}' schedule task. {1}", this.Name, exc.Message), exc);
