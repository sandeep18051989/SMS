...
namespace EF.Services.Service
{
    public partial class PictureService : IPictureService
    {
        ...
        public IList<Picture> GetAllPictures(bool? onlyActive = null, bool? onlyOpenResource = null)
        {
            return _pictureRepository.Table.Where(x => (!onlyActive.HasValue || x.IsActive == onlyActive.Value) && (!onlyOpenResource.HasValue || x.IsOpenResource == onlyOpenResource.Value)).ToList();
        }
        public IList<Picture> GetAllPicturesByUser(int userId, bool? onlyActive = null, bool? onlyOpenResource = null)
        {
            return _pictureRepository.Table.Where(x => x.UserId == userId && (!onlyActive.HasValue || x.IsActive == onlyActive.Value) && (!onlyOpenResource.HasValue || x.IsOpenResource == onlyOpenResource.Value)).ToList();
        }
        ...
        public void TogglePicture(int id)
        {
            ...
            var picture = _pictureRepository.Table.FirstOrDefault(x => x.Id == id);
            if (picture != null)
            {
        ...
        public void ToggleActiveStatusPicture(int id)
        {
            if (id == 0)
                throw new ArgumentNullException("id");

            var objPicture = _pictureRepository.GetByID(id);
            if (objPicture != null)
            {
                objPicture.IsActive = !objPicture.IsActive;
                objPicture.ModifiedOn = DateTime.Now;
                _pictureRepository.Update(objPicture);
            }
        }
