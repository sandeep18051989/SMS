...
namespace EF.Services.Service
{
	public class CustomPageService : ICustomPageService
	{
		...
		public IList<CustomPage> GetAllCustomPages(bool active = false)
		{
			if (active)
				return _customPageRepository.Table.Where(x => x.IsActive).ToList();

			return _customPageRepository.Table.ToList();
		}
