...
namespace EF.Services.Http
{
	public static class UrlExtensions
	{
		...
		public static string GetSystemName(string name)
		{
			return GetSystemName(name, true, true);
		}
			 ...
			 bool ensureTwoPublishedLanguages = true)
		{
			...
			{
				result = urlRecordService.GetSlug(entityId, entityName);
			}

			return result;
		}

		public static string ValidateSystemName<T>(this T entity, string systemName, string name, bool ensureNotEmpty)
				 where T : BaseEntity, ISlugSupported
		{
			if (entity == null)
				throw new ArgumentNullException("entity");

			return ValidateSystemName(entity.Id, typeof(T).Name, systemName, name, ensureNotEmpty);
		}

		public static string ValidateSystemName(int entityId, string entityName, string systemName, string name, bool ensureNotEmpty)
		{
			if (String.IsNullOrWhiteSpace(systemName) && !String.IsNullOrWhiteSpace(name))
				systemName = name;

			systemName = GetSystemName(systemName);

			systemName = CommonHelper.EnsureMaximumLength(systemName, 200);

			if (String.IsNullOrWhiteSpace(systemName))
			{
				if (ensureNotEmpty)
				{
					systemName = entityId.ToString();
				}
				else
				{
					return systemName;
			...
			while (true)
			{
				...
				tempSystemName = string.Format("{0}-{1}", systemName, i);
				i++;
			}
		...
		public static string GetSystemName(string name, bool convertNonWesternChars, bool allowUnicodeCharsInUrls)
		{
